...
################################################################################
Function:
  Global alignment between two sequences by using Needleman-Wunsch algorithm.

Usage:
  perl Needleman_Wunsch.pl -f <fasta> -o <output> -m <match> -u <unmatch> -s <space>
...
#step1 : start global alignment
my $id_seq = GetFastaInfo($fasta);
my @ids    = sort keys %$id_seq;
my $time1  = join '.', gettimeofday; #record the start time
my ($max_align_score, $align_seq_1, $align_seq_2) = NeedlemanWunsch( $id_seq->{$ids[0]}, $id_seq->{$ids[1]} );
...
printf "the run time of NW algorithm is :\t%.4fs\n", $time2 - $time1;
print  "----------------------------------------------------------------------\n";
...
sub NeedlemanWunsch {
    ...
    #note : in arrow matrix, -1 mean left, 1 mean up, and 0 mean upper left
    foreach my $i (0 , 1) {
        foreach my $j (2 .. $#{$matrix_score[$i]}) {
            $matrix_score[$i][$j] = $i == 0 ? substr $seq2, $j - 2, 1 : $matrix_score[$i][$j-1] + $space;
            $matrix_arrow[$i][$j] = $i == 0 ? substr $seq2, $j - 2, 1 : -1;
        }
    }
    foreach my $j (0 , 1) {
        foreach my $i (2 .. $#matrix_score) {
            $matrix_score[$i][$j] = $j == 0 ? substr $seq1, $i - 2, 1 : $matrix_score[$i-1][$j] + $space;
            $matrix_arrow[$i][$j] = $j == 0 ? substr $seq1, $i - 2, 1 : 1;
        }
    }
    ...
    #step3----------------------------------------------------------------------
    #get a maximum alignment score and two aligned sequences
    my $max_align_score = $matrix_score[$#matrix_score][$#{$matrix_score[$#matrix_score]}];
    my ($align_seq_1, $align_seq_2);
    my ($i, $j) = ($#matrix_arrow, $#{$matrix_arrow[$#matrix_arrow]});
    until ($i == 1 and $j == 1) {
